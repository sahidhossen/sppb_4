...
class SelectControl extends Component {
  onChange(event) {
    if (Array.isArray(this.props.value)) {
      const defaultValue = [...new Set(this.props.value.reduce((acc, value) => acc.concat(value), []))];
      const value = [...new Set([...defaultValue, ...[...event.target.selectedOptions].map((o) => o.value)])];
  ...
  render() {
    ...
    const renderOptions = options.map((option, index) => {
      return (
        <option key={index} value={option.value} disabled={option.disabled ? true : null}>
          {option.label}
        </option>
      );
    });
