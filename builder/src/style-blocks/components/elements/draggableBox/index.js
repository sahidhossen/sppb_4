...
class DraggableBox extends React.Component {
  ...
  onDragOverAction(event) {
    ...
    let { value, direction, options = {}, onDragChange } = this.props;
    ...
    if (direction === "VL" || direction === "VR") {
      ...
      if (options.abs) nextValue = Math.abs(nextValue);
    }
  ...
  render() {
    let { className, activeClass, options = {} } = this.props;
    let classNames = classnames(className, {
      [activeClass]: options.isActive || options.isLocked,
    });
    return (
      <div className={classNames} onClick={this.onBoxClick.bind(this)} onMouseDown={this.onMouseDownAction.bind(this)}>
        {this.props.children}
      </div>
    );
  }
