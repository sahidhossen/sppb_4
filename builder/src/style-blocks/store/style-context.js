...
export class StyleBlockContextProvider extends React.Component {
  ...
  render() {
    return <Provider value={{ state: this.state, dispatch: this.dispatch }}>{this.props.children}</Provider>;
  }
}

export const withStyleContext = (mapContextToProps) =>
  createHigherOrderComponent((OriginalComponent) => {
    return (props) => (
      <Consumer>{(context) => <OriginalComponent {...props} {...mapContextToProps(context, props)} />}</Consumer>
    );
  }, "withStyleContext");
